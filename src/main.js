...
const ContractAddress = "0x1D2F9d30d59B0aBDd7eA8AA085E8f497C0526778";
let kit
let contract
let campaigns = []
...
const connectCeloWallet = async function () {
    if (window.celo) {
        ...
        try {
            ...
            contract = new kit.web3.eth.Contract(highfiveAbi, ContractAddress)
        } catch (error) {
...
function renderCampaigns() {
    document.getElementById("highfive").innerHTML = ""
    campaigns.forEach((_campaign) => {
        const newDiv = document.createElement("div")
        newDiv.className = "col-md-4"
        newDiv.innerHTML = campaignTemplate(_campaign)
        document.getElementById("highfive").appendChild(newDiv)
    })
}
...
document.querySelector("#highfive").addEventListener("click", async (e) => {
